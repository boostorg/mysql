...
public:
    ...
    {
        return impl([&] { return this->obj().read_one(use_views); });
    }
    network_result<rows_view> read_some() override
    {
        return impl([&] { return this->obj().read_some(use_views); });
    }
    network_result<rows_view> read_all() override
    {
        return impl([&] { return this->obj().read_all(use_views); });
    }
};
...
public:
    ...
    {
        return impl([&] {
            this->obj().execute(std::make_tuple(param), opts, this->cast(result));
            return no_result();
        });
    }
    ...
    {
        return impl([&] {
            this->obj().execute(std::make_tuple(param1, param2), this->cast(result));
            return no_result();
        });
    }
    ...
    {
        return impl([&] {
            this->obj().execute(params_first, params_last, opts, this->cast(result));
            return no_result();
        });
    }
