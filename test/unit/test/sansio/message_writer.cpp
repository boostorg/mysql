...
BOOST_AUTO_TEST_CASE(regular_message)
{
    ...
    // Operation start
    processor.prepare_write(mock_message{msg_body}, seqnum);
    BOOST_TEST(!processor.done());

    // First (and only) chunk
    auto chunk = processor.current_chunk();
...
BOOST_AUTO_TEST_CASE(empty_message)
{
    ...
    // Operation start
    processor.prepare_write(mock_message{}, seqnum);
    BOOST_TEST(!processor.done());
...
BOOST_AUTO_TEST_CASE(multiframe_message)
{
    ...
    // Operation start
    processor.prepare_write(mock_message{msg}, seqnum);
    BOOST_TEST(!processor.done());
...
BOOST_AUTO_TEST_CASE(several_messages)
{
    ...
    processor.prepare_write(mock_message{msg_1}, seqnum_1);
    BOOST_TEST(!processor.done());
    ...
    processor.prepare_write(mock_message{msg_2}, seqnum_2);
    BOOST_TEST(!processor.done());
    ...
    processor.prepare_write(mock_message{msg_3}, seqnum_3);
    BOOST_TEST(!processor.done());
