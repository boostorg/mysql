...
{
    output.push_back(success_sample("fractional_negative", {0x66, 0x66, 0x86, 0xc0},
            -4.2f, protocol_field_type::float_));
    output.push_back(success_sample("fractional_positive", {0x66, 0x66, 0x86, 0x40},
            4.2f, protocol_field_type::float_));
    output.push_back(success_sample("positive_exp_positive_fractional", {0x01, 0x2d, 0x88, 0x61},
            3.14e20f, protocol_field_type::float_));
    output.push_back(success_sample("zero", {0x00, 0x00, 0x00, 0x00},
            0.0f, protocol_field_type::float_));
}

void add_double_samples(std::vector<success_sample>& output)
{
    output.push_back(success_sample("fractional_negative",
        {0xcd, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0x10, 0xc0},
        -4.2, protocol_field_type::double_));
    output.push_back(success_sample("fractional_positive",
        {0xcd, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0x10, 0x40},
        4.2, protocol_field_type::double_));
    output.push_back(success_sample("positive_exp_positive_fractional",
        {0xce, 0x46, 0x3c, 0x76, 0x9c, 0x68, 0x90, 0x69},
        3.14e200, protocol_field_type::double_));
    output.push_back(success_sample("zero",
...
{
    output.push_back(success_sample("regular", {0x04, 0xda, 0x07, 0x03, 0x1c},
            makedate(2010, 3, 28), protocol_field_type::date));
    output.push_back(success_sample("min", {0x04, 0x00, 0x00, 0x01, 0x01},
            makedate(0, 1, 1), protocol_field_type::date));
    output.push_back(success_sample("max", {0x04, 0x0f, 0x27, 0x0c, 0x1f},
            makedate(9999, 12, 31), protocol_field_type::date));
    output.push_back(success_sample("zero", {0x00},
            nullptr, protocol_field_type::date));
    output.push_back(success_sample("zero_full_length", {0x04, 0x00, 0x00, 0x00, 0x00},
            nullptr, protocol_field_type::date));
    output.push_back(success_sample("zero_month",   {0x04, 0x00, 0x00, 0x00, 0x01},
            nullptr, protocol_field_type::date));
    output.push_back(success_sample("zero_day",     {0x04, 0x00, 0x00, 0x01, 0x00},
            nullptr, protocol_field_type::date));
    output.push_back(success_sample("zero_month_day_nonzero_year", {0x04, 0x01, 0x00, 0x00, 0x00},
            nullptr, protocol_field_type::date));
    output.push_back(success_sample("invalid_date", {0x04, 0x00, 0x00,   11,   31},
            nullptr, protocol_field_type::date));
}
...
struct error_sample
{
    ...
    error_sample(std::string&& name, bytestring&& from, protocol_field_type type,
...
{
    output.emplace_back(error_sample("signed_not_enough_space",
        bytestring(num_bytes, 0x0a), type, errc::incomplete_message));
    output.emplace_back(error_sample("unsigned_not_enough_space",
...
{
    output.emplace_back(error_sample("bit_error_deserializing_string_view",
        {0x01}, protocol_field_type::bit, column_flags::unsigned_, errc::incomplete_message));
    output.emplace_back(error_sample("bit_string_view_too_short",
        {0x00}, protocol_field_type::bit, column_flags::unsigned_));
    output.emplace_back(error_sample("bit_string_view_too_long",
...
{
    output.push_back(error_sample("not_enough_space", {0x01, 0x02, 0x03},
            protocol_field_type::float_, errc::incomplete_message));
    output.push_back(error_sample("inf", {0x00, 0x00, 0x80, 0x7f},
            protocol_field_type::float_));
    output.push_back(error_sample("minus_inf", {0x00, 0x00, 0x80, 0xff},
            protocol_field_type::float_));
    output.push_back(error_sample("nan", {0xff, 0xff, 0xff, 0x7f},
            protocol_field_type::float_));
    output.push_back(error_sample("minus_nan", {0xff, 0xff, 0xff, 0xff},
            protocol_field_type::float_));
}

void add_double_samples(std::vector<error_sample>& output)
{
    output.push_back(error_sample("not_enough_space", {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07},
            protocol_field_type::double_, errc::incomplete_message));
    output.push_back(error_sample("inf", {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x7f},
            protocol_field_type::double_));
    output.push_back(error_sample("minus_inf", {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0xff},
            protocol_field_type::double_));
    output.push_back(error_sample("nan", {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f},
            protocol_field_type::double_));
    output.push_back(error_sample("minus_nan", {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff},
            protocol_field_type::double_));
}
...
{
    output.push_back(error_sample("empty", {},
        protocol_field_type::date, errc::incomplete_message));
    output.push_back(error_sample("incomplete_year",   {0x04, 0xda},
            protocol_field_type::date, errc::incomplete_message));
    output.push_back(error_sample("no_month_day",      {0x04, 0xda, 0x07},
            protocol_field_type::date, errc::incomplete_message));
    output.push_back(error_sample("no_day",            {0x04, 0xda, 0x07, 0x03},
            protocol_field_type::date, errc::incomplete_message));
    output.push_back(error_sample("invalid_year",      {0x04, 0x10, 0x27, 0x03, 0x1c}, // year 10000
            protocol_field_type::date));
    output.push_back(error_sample("invalid_year_max",  {0x04, 0xff, 0xff, 0x03, 0x1c},
            protocol_field_type::date));
    output.push_back(error_sample("invalid_month",     {0x04, 0xda, 0x07,   13, 0x1c},
            protocol_field_type::date));
    output.push_back(error_sample("invalid_month_max", {0x04, 0xda, 0x07, 0xff, 0x1c},
            protocol_field_type::date));
    output.push_back(error_sample("invalid_day",       {0x04, 0xda, 0x07, 0x03,   32},
            protocol_field_type::date));
    output.push_back(error_sample("invalid_day_max",   {0x04, 0xda, 0x07, 0x03, 0xff},
            protocol_field_type::date));
    output.push_back(error_sample("protocol_max",      {0xff, 0xff, 0xff, 0xff, 0xff},
            protocol_field_type::date));
}
...
void add_time_samples(std::vector<error_sample>& output)
{
    ...
    output.push_back(error_sample("empty",
        {}, type, errc::incomplete_message));
    output.push_back(error_sample("no_sign_days_hours_mins_secs",
        {0x08}, type, errc::incomplete_message));
    output.push_back(error_sample("no_days_hours_mins_secs",
    ...
    output.push_back(error_sample("no_micros",
