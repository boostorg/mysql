...
using netfun_maker = netfun_maker_mem<void, message_writer, any_stream&>;
...
struct
{
    ...
    {netfun_maker::sync_errc_noerrinfo(&message_writer::write),   "sync" },
    {netfun_maker::async_noerrinfo(&message_writer::async_write), "async"},
};
...
BOOST_AUTO_TEST_CASE(success)
{
    for (auto fns : all_fns)
    {
        BOOST_TEST_CONTEXT(fns.name)
        {
            ...
            // Write
            fns.write(fix.writer, fix.stream).validate_no_error();
...
BOOST_AUTO_TEST_CASE(multi_frame)
{
    for (auto fns : all_fns)
    {
        BOOST_TEST_CONTEXT(fns.name)
        {
            ...
            auto expected_msg = concat_copy(create_frame(4, msg_frame_1), create_frame(5, msg_frame_2));
...
BOOST_AUTO_TEST_CASE(error)
{
    for (auto fns : all_fns)
    {
        BOOST_TEST_CONTEXT(fns.name)
        {
            ...
            // Write
            fns.write(fix.writer, fix.stream).validate_error_exact(client_errc::server_unsupported);
        }
    }
}
