...
void main_impl(int argc, char** argv)
{
    ...
    boost::asio::co_spawn(
        ...
        [&conn, &params, company_id]() -> boost::asio::awaitable<void> {
            ...
            boost::mysql::pipeline pipe;
            pipe.add_execute("START TRANSACTION")
            ...
            auto id1 = pipe.steps()[1].execute_result().last_insert_id();
            auto id2 = pipe.steps()[2].execute_result().last_insert_id();
            auto id3 = pipe.steps()[3].execute_result().last_insert_id();

            // If the above statement were successful, we can close the statements
            // and run the COMMIT statement
            pipe.clear();
            pipe.add_close_statement(stmts.at(0)).add_close_statement(stmts.at(1)).add_execute("COMMIT");
