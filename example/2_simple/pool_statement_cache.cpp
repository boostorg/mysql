...
class statement_cache_state
{
    std::unordered_map<std::string_view, mysql::statement> statements_;
    mysql::any_connection& conn_;

public:
    statement_cache_state(mysql::any_connection& conn) : conn_(conn) {}
    void reset() { statements_.clear(); }

    asio::awaitable<mysql::statement> prepare_statement(mysql::constant_string_view str)
    {
        auto it = statements_.find(str.get());
        if (it != statements_.end())
            co_return it->second;
        auto stmt = co_await conn_.async_prepare_statement(str.get());
        statements_.insert({str.get(), stmt});
        co_return stmt;
    }
};

using cached_pool = mysql::basic_connection_pool<statement_cache_state>;
...
asio::awaitable<std::string> get_employee_details(cached_pool& pool, std::int64_t employee_id)
{
    ...
    // Prepare a statement if required
    auto stmt = co_await conn.node().prepare_statement(
...
asio::awaitable<void> handle_session(cached_pool& pool, asio::ip::tcp::socket client_socket)
{
...
asio::awaitable<void> listener(cached_pool& pool, unsigned short port)
{
...
void main_impl(int argc, char** argv)
{
    ...
    cached_pool pool(ctx, std::move(params));
