...
int main()
{
    ...
    // Prepare stmt
    auto stmt = conn.prepare_statement("SELECT * FROM myt WHERE id = 1");
    ...
    // std::cout << "Elapsed: " << std::chrono::duration_cast<std::chrono::milliseconds>(tend -
    // tbegin).count()
    //           << "ms" << std::endl;
