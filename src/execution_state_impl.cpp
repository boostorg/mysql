...
boost::mysql::error_code boost::mysql::detail::execution_state_impl::on_head_ok_packet_impl(
    const ok_view& pack,
    diagnostics&
)
{
...
boost::mysql::error_code boost::mysql::detail::execution_state_impl::on_meta_impl(
...
{
    // add row storage
    std::span<field_view> storage = add_fields(fields, meta_.size());
