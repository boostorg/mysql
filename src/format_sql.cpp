...
#include "mycosql_internal/byte_to_hex.hpp"
...
inline void append_quoted_identifier(std::string_view name, format_context_base& ctx)
{
    ctx.append_raw("`");
...
void append_int(T integer, format_context_base& ctx)
{
    ...
    // Copy
    access::get_impl(ctx).output.append(std::string_view(buff, res.ptr - buff));
}
...
inline void append_quoted_string(std::string_view str, format_context_base& ctx)
{
    auto& impl = access::get_impl(ctx);
    impl.output.append("'");
...
inline void append_string(std::string_view str, std::string_view format_spec, format_context_base& ctx)
{
    // Parse format spec
...
inline void append_quoted_date(date d, format_context_base& ctx)
{
    ...
    buffer[0] = '\'';
    std::size_t
        sz = detail::date_to_string(d.year(), d.month(), d.day(), std::span<char, 32>(buffer + 1, 32));
    buffer[sz + 1] = '\'';
    access::get_impl(ctx).output.append(std::string_view(buffer, sz + 2));
}
...
inline void append_quoted_time(time t, format_context_base& ctx)
{
    char buffer[66];
    buffer[0] = '\'';
    std::size_t sz = time_to_string(t, std::span<char, 64>(buffer + 1, 64));
    buffer[sz + 1] = '\'';
    access::get_impl(ctx).output.append(std::string_view(buffer, sz + 2));
}
...
class format_state
{
    ...
    void do_field(format_arg arg, std::string_view format_spec)
    {
        ctx_.format_arg(access::get_impl(arg).value, format_spec);
    }

    BOOST_ATTRIBUTE_NODISCARD
    bool do_indexed_field(int arg_id, std::string_view format_spec)
    {
    ...
    struct arg_id_t
    {
        ...
        arg_id_t(std::string_view v) noexcept : type(type_t::identifier) { data.identifier = v; }
    };
    ...
    static arg_id_t parse_arg_id(const char*& it, const char* format_end)
    {
        ...
        else if (is_name_start(*it))
        {
            ...
            while (it != format_end && (is_name_start(*it) || is_number(*it)))
                ++it;
            std::string_view field_name(name_begin, it);
            return field_name;
        }
        else
        {
    ...
    BOOST_ATTRIBUTE_NODISCARD
    static std::string_view parse_format_spec(const char*& it, const char* format_end)
    {
        if (it != format_end && *it == ':')
        {
            ++it;
    ...
    BOOST_ATTRIBUTE_NODISCARD
    bool append_named_field(std::string_view field_name, std::string_view format_spec)
    {
        // Find the argument
    ...
    BOOST_ATTRIBUTE_NODISCARD
    bool append_indexed_field(int index, std::string_view format_spec)
    {
        if (uses_auto_ids())
        {
    ...
    BOOST_ATTRIBUTE_NODISCARD
    bool append_auto_field(std::string_view format_spec)
    {
...
public:
    format_state(format_context_base& ctx, std::span<const format_arg> args) noexcept : ctx_(ctx), args_(args)
    {
    }
...
void boost::mysql::format_context_base::format_arg(
    detail::formattable_ref_impl arg,
    std::string_view format_spec
)
{
    switch (arg.type)
...
std::pair<bool, std::string_view> boost::mysql::detail::parse_range_specifiers(
