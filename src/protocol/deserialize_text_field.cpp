...
#include "protocol/bit_deserialization.hpp"
...
static constexpr std::size_t year_sz = 4;
static constexpr std::size_t month_sz = 2;
static constexpr std::size_t day_sz = 2;
static constexpr std::size_t hours_min_sz = 2;  // in TIME, it may be longer
static constexpr std::size_t mins_sz = 2;
static constexpr std::size_t secs_sz = 2;

static constexpr std::size_t date_sz = year_sz + month_sz + day_sz + 2;           // delimiters
static constexpr std::size_t time_min_sz = hours_min_sz + mins_sz + secs_sz + 2;  // delimiters
static constexpr std::size_t time_max_sz = time_min_sz + max_decimals +
                                           3;  // sign, period, hour extra character
static constexpr std::size_t datetime_min_sz = date_sz + time_min_sz + 1;           // delimiter
static constexpr std::size_t datetime_max_sz = datetime_min_sz + max_decimals + 1;  // period

static constexpr unsigned time_max_hour = 838;

// Integers
template <class T>
static deserialize_errc deserialize_text_value_int_impl(string_view from, field_view& to) noexcept
{
    T v;
...
static deserialize_errc deserialize_text_value_int(
    string_view from,
    field_view& to,
    const metadata& meta
) noexcept
...
// Floating points
template <class T>
static deserialize_errc deserialize_text_value_float(string_view from, field_view& to) noexcept
{
    T val;
...
// Strings
static deserialize_errc deserialize_text_value_string(string_view from, field_view& to) noexcept
{
    to = field_view(from);
    return deserialize_errc::ok;
}
...
// Date/time types
static unsigned sanitize_decimals(unsigned decimals) noexcept { return (std::min)(decimals, max_decimals); }

// Computes the meaning of the parsed microsecond number, taking into
// account decimals (85 with 2 decimals means 850000us)
static unsigned compute_micros(unsigned parsed_micros, unsigned decimals) noexcept
{
...
deserialize_errc boost::mysql::detail::deserialize_text_field(
    string_view from,
    const metadata& meta,
    field_view& output
)
