...
// OK packets
error_code boost::mysql::detail::deserialize_ok_packet(span<const std::uint8_t> msg, ok_view& output) noexcept
{
    struct ok_packet
    {
...
// Error packets
error_code boost::mysql::detail::deserialize_error_packet(
...
void boost::mysql::detail::prepare_stmt_command::serialize(asio::mutable_buffer buff) const noexcept
{
    ...
    BOOST_ASSERT(buff.size() >= get_size());
    serialization_context ctx(buff.data());
    ::serialize(ctx, command_id, string_eof{stmt});
}

error_code boost::mysql::detail::deserialize_prepare_stmt_response_impl(
...
std::size_t boost::mysql::detail::login_request::get_size() const noexcept
{
    ...
    return ::get_size(
           ...
           (negotiated_capabilities.has(CLIENT_CONNECT_WITH_DB) ? ::get_size(pack.database) : 0) +
...
void boost::mysql::detail::login_request::serialize(asio::mutable_buffer buff) const noexcept
{
    ...
    {
        ::serialize(ctx, pack.database);
    }
    ::serialize(ctx, pack.client_plugin_name);
}
...
error_code boost::mysql::detail::deserialize_auth_switch(
    span<const std::uint8_t> msg,
    auth_switch& output
) noexcept
{
...
void boost::mysql::detail::auth_switch_response::serialize(asio::mutable_buffer buff) const noexcept
{
    BOOST_ASSERT(buff.size() >= get_size());
    serialization_context ctx(buff.data());
    ::serialize(ctx, string_eof{to_string(auth_plugin_data)});
}
