...
#include "mycosql_internal/sansio/auth_plugin_common.hpp"
...
inline void csha2p_hash_password_impl(
    ...
    std::span<const std::uint8_t, scramble_size> scramble,
    std::span<std::uint8_t, csha2p_hash_size> output
)
{
...
)
{
    ...
    csha2p_hash_password_impl(
        password,
        scramble,
        std::span<std::uint8_t, csha2p_hash_size>(res.data(), csha2p_hash_size)
    );
    return res;
}
...
class csha2p_algo
{
    ...
    static bool is_perform_full_auth(std::span<const std::uint8_t> server_data)
    {
        return server_data.size() == 1u && server_data[0] == 4;
    }

    static bool is_fast_auth_ok(std::span<const std::uint8_t> server_data)
    {
    ...
    static next_action encrypt_password(
        ...
        std::string_view password,
        std::span<const std::uint8_t, scramble_size> scramble,
        std::span<const std::uint8_t> server_key
    )
    {
        ...
        if (ec)
            return ec;
        return st.write(
            string_eof{std::string_view(reinterpret_cast<const char*>(buff.data()), buff.size())},
            seqnum
        );
    }
...
public:
    ...
    next_action resume(
        connection_state_data& st,
        std::span<const std::uint8_t> server_data,
        std::string_view password,
        std::span<const std::uint8_t, scramble_size> scramble,
        bool secure_channel,
        std::uint8_t& seqnum
