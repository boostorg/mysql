...
public:
    ...
    std::span<std::uint8_t> buffer() { return buffer_.free_area(); }
    ...
    error_code prepare_buffer()
    {
        ...
        if (occupied_space <= small_move_threshold || (state_.required_size > buffer_.free_size()))
        {
    ...
    void resume(std::size_t bytes_read)
    {
        ...
        case 0:
            ...
            while (true)
            {
                ...
                header = deserialize_frame_header(
                    std::span<const std::uint8_t, frame_header_size>(
