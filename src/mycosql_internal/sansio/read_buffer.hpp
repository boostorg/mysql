...
#include "mycosql_internal/next_power_of_two.hpp"
...
public:
    ...
    std::span<const std::uint8_t> reserved_area() const noexcept
    {
        return {reserved_first(), reserved_size()};
    }
    std::span<const std::uint8_t> current_message() const noexcept
    {
        return {current_message_first(), current_message_size()};
    }
    std::span<const std::uint8_t> pending_area() const noexcept { return {pending_first(), pending_size()}; }
    std::span<std::uint8_t> free_area() noexcept { return {free_first(), free_size()}; }
