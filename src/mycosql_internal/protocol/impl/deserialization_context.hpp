...
public:
    explicit deserialization_context(std::span<const std::uint8_t> data) noexcept
    ...
    std::string_view get_string(std::size_t sz) const
    {
        return std::string_view(reinterpret_cast<const char*>(first_), sz);
    }
    ...
    std::span<const std::uint8_t> to_span() const { return std::span<const std::uint8_t>(first_, last_); }
