...
class serialization_context
{
    ...
    void append_to_buffer(std::span<const std::uint8_t> contents)
    {
    ...
    void add_impl(std::span<const std::uint8_t> content)
    {
...
public:
    ...
    void add(std::span<const std::uint8_t> content) { add_impl(content); }
    ...
    std::uint8_t write_frame_headers(std::uint8_t seqnum, std::size_t initial_offset)
    {
        ...
        while (offset < buffer_.size())
        {
            ...
            serialize_frame_header(
                std::span<std::uint8_t, frame_header_size>(buffer_.data() + offset, frame_header_size),
