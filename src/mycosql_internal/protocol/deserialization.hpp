...
#include "mycosql_internal/error/server_error_to_string.hpp"
...
inline error_code deserialize_ok_packet(std::span<const std::uint8_t> msg, ok_view& output);  // for testing
...
struct row_message
{
    ...
    union data_t
    {
        ...
        data_t(std::span<const std::uint8_t> row) noexcept : row(row) {}
        data_t(const ok_view& ok_pack) noexcept : ok_pack(ok_pack) {}
        data_t(error_code err) noexcept : err(err) {}
    } data;

    row_message(std::span<const std::uint8_t> row) noexcept : type(type_t::row), data(row) {}
...
inline error_code deserialize_row(
    ...
    std::span<field_view> output  // Should point to meta.size() field_view objects
);
...
struct handshake_server_response
{
    ...
    union data_t
    {
        ...
        error_code err;
        auth_switch auth_sw;
        std::span<const std::uint8_t> more_data;

        data_t(const ok_view& ok) noexcept : ok(ok) {}
        data_t(error_code err) noexcept : err(err) {}
        data_t(auth_switch msg) noexcept : auth_sw(msg) {}
        data_t(std::span<const std::uint8_t> more_data) noexcept : more_data(more_data) {}
    } data;
    ...
    handshake_server_response(std::span<const std::uint8_t> more_data) noexcept
...
)
{
    ...
    // Error message
    access::get_impl(diag).assign_server(std::string(error_packet.error_message));
...
)
{
    ...
    else
    {
        ...
        return std::span<const std::uint8_t>(ctx.first(), ctx.size());
    }
}
...
inline db_flavor parse_db_version(std::string_view version_string)
{
    return version_string.find("MariaDB") != std::string_view::npos ? db_flavor::mariadb : db_flavor::mysql;
}
...
)
{
    ...
    auto auth2_length = static_cast<std::uint8_t>((std::max)(static_cast<std::size_t>(13u),
                                                             static_cast<std::size_t>(
                                                                 pack.auth_plugin_data_len.value -
    ...
    output.auth_plugin_data.append(std::span<const std::uint8_t>(auth2_data, auth2_length - 1));
...
)
{
    ...
    std::string_view auth_data = pack.auth_plugin_data.value;
