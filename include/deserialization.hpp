...
// Utility
inline void check_size(ReadIterator from, ReadIterator last, std::size_t sz)
{
	if ((last - from) < sz)
...
inline ReadIterator deserialize(ReadIterator from, ReadIterator last, int_lenenc& output)
{
	std::uint8_t first_byte = (from++)[0];
	if (first_byte == 0xFC)
	{
		int2 value;
...
inline ReadIterator deserialize(ReadIterator from, ReadIterator last, string_null& output)
{
