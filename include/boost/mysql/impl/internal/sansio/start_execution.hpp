...
class start_execution_algo
{
    ...
    next_action compose_request()
    {
        if (req_.is_query)
        {
            return conn_state().write(query_command{req_.data.query}, seqnum());
        }
        else
        {
...
public:
    start_execution_algo(connection_state_data& st, start_execution_algo_params params) noexcept
        : read_head_st_(st, read_resultset_head_algo_params{params.diag, params.proc}), req_(params.req)
    {
    }
    ...
    next_action resume(error_code ec)
    {
        ...
        case 0:
            ...
            // Reset the processor
            processor().reset(get_encoding(req_), conn_state().meta_mode);
